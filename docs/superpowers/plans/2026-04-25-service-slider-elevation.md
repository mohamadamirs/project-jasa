# Service Slider Elevation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement a horizontal snapping slider for the services section with an "Elevation" focus, avoiding dark overlays on inactive cards.

**Architecture:** Use a React Island (`ServiceSlider.tsx`) with scroll-position tracking to apply dynamic styling based on card proximity to the viewport center.

**Tech Stack:** Astro 6, React 19, Tailwind v4, Lucide React.

---

### Task 1: Update ServiceSlider Component

**Files:**
- Modify: `src/components/react/ServiceSlider.tsx`

- [ ] **Step 1: Write the updated component code**
Hapus efek `opacity` rendah dan `bg-white/50`. Gunakan `bg-white` murni untuk semua kartu. Gunakan `-translate-y-4` dan bayangan kuat untuk kartu aktif.
...
Run: `git add src/components/react/ServiceSlider.tsx && git commit -m "feat(ux): implement elevation-based service slider without dark overlays"`

---

### Task 2: Final Verification

- [ ] **Step 1: Verify index.astro usage**
Pastikan `src/pages/index.astro` mengimpor dan menggunakan `ServiceSlider` dengan `client:visible`.
